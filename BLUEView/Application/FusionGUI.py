...
def _open_sam_file():
    ...
    if file:
        try:
            ...
            insert_reads(init.reads) # Load in read information into the display
            sam_open = True
        except:
...
def insert_reads(reads):
    for read in reads:
        ...
        if read['supplement'] != "No Fusions Detected":
            fusion_area.insert(INSERT, "\t" + read['fusion'])
            fusion_area.insert(INSERT, '\n')

            len_diff = read['fusionLength'] - read['supplementLength']
            fusion_area.tag_config(read['fusion'], background='pale green')
            fusion_area.tag_config(read['supplement'], background='light salmon')
            # Highlights the fusion and supplement gene
            search(fusion_area, read['fusion'], read['fusion'])
            search(fusion_area, read['supplement'], read['supplement'])
        ...
        if read['fusionName'] != "" and read['fusionName'] not in fusion_genes:
            add_gene_info(read['fusionName'], read['supplementName'], read['fusionId'], read['supplement'])
            add_fusion_summary(read['fusionName'], read['supplementName'], read['fusionPosition'], read['supplementPosition'], read['fusionLength'], read['supplementLength'])
