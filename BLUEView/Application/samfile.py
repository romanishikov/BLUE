...
def map_fusion_reads():
    ...
    if i > 1:
        while x < len(all_positions):
            ...
            if all_positions[left_indx] != all_positions[right_indx] and left_indx not in paired_reads and right_indx not in paired_reads:
                read_dict['fusion'], fusion_indx, read_dict['supplement'], supp_indx = get_fusion_read(left_indx, right_indx)
                ...
                if read_dict['fusion'] != "No Fusions Detected":
                    ...
                    if total_fusions == 1:
                        ...
                        read_dict['headers'] = get_position_headers(read_dict['fusionLength'], read_dict['fusionPosition'], 50)
                    ...
                    try:
                        ...
                        read_dict['fusionName'] = get_gene_names(read_dict['fusionLength'], read_dict['fusionPosition'], fusion_chr)
                        read_dict['fusionId'] = get_gene_ids(read_dict['fusionLength'], read_dict['fusionPosition'], fusion_chr)[0]

                        read_dict['supplementName'] = get_gene_names(read_dict['supplementLength'], read_dict['supplementPosition'], supp_chr)
                        read_dict['supplementId'] = get_gene_ids(read_dict['supplementLength'], read_dict['supplementPosition'], supp_chr)[0]
    ...
    if total_fusions == 0: FUSION_READS.append(dict(read_dict)) # If no fusions found display one instance of alert
